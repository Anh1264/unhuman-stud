...
export const metadata: Metadata = {
  title: "Films",
  description: "Original short films and cuts from Unhuman Stud.",
};
      ...
      {hero && (
        <Reveal>
          ...
          <div className="mt-5 flex flex-wrap items-baseline justify-between gap-3">
            <div>
              <h3 className="serif text-[27px]">{hero.title}</h3>
              {hero.description && (
      ...
      {rest.length > 0 && (
        <>
          <div className="mt-16 mb-6 flex flex-wrap items-baseline justify-between gap-2">
            <h3 className="serif text-[22px]">More films</h3>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {rest.map((film, i) => (
              <Reveal key={film.slug} delay={i * 60}>
                ...
                <h4 className="serif mt-3 text-[18px]">{film.title}</h4>
                {film.projectTitle && (
